...
const ViewScore = () => {
  ...
  const {isLoading,data}=useQuery("fetchstudent",()=>fetchStudent(get().pid),{
    onSuccess: (data) => {
      const arr = data?.data[0].assignedQuiz.filter(item => item.status === "Attempted");
  ...
  if(!isLoading){
    var arr = data?.data[0].assignedQuiz.filter(item => item.status === "Attempted");
